...
dat=h5dump(paste0(path_files,"data/data_O3.h5"))
Ngd=256
...
for(id_case in 1:3){
  ...
  filenames=c(paste0("VMCMC/","data_O3_",id_case-1,".h5"),
              paste0("VAE/","data_O3_",id_case-1,".h5"),
              paste0("MGDM/","data_O3_",id_case-1,".h5"))
  dataname=rev(c("samples","samples", "samples"))
  
  ttl=paste0("~/Documents/Work/doc/diff_post_gauss_paper/images/map_crps_O3_case_",id_case,".png")
  ...
  for(id_gen in 1:length(method_name)){
    ...
    tryCatch({
      ...
      vs_tab[id_case,3+id_gen]=sd(sco)
      ...
      vsw_tab[id_case,3+id_gen]=sd(sco)
...
vs_val[,1]=0
for(i in 1:n_cases){
  vs_val[i,(1:n_methods)+2]=paste0(round(vs_tab[i,1:n_cases],ndig)," (",round(vs_tab[i,n_cases+1:n_cases],ndig),") | ")
}
...
vsw_val[,1]=0
for(i in 1:n_cases){
  vsw_val[i,(1:n_methods)+2]=paste0(round(vsw_tab[i,1:n_cases],ndig)," (",round(vsw_tab[i,n_cases+1:n_cases],ndig),") | ")
}
